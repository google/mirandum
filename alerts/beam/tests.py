...
from beam.models import SponsorUpdate, SponsorEvent
from googaccount.models import AppCreds
from django.contrib.auth.models import User

from beam.support import run_sponsors
...
class InsertBeam(TestCase):
    ...
    def testInsertion(self):
        ...
        def sponsor_data(*args):
            return create_beam(['a', 'b'])
        run_beam(self.updater, sponsor_data)
        
        def sponsor_data(*args):
            return create_beam(['d', 'c', 'a', 'b'])

        run_beam(self.updater, sponsor_data)
        self.assertEqual(SponsorEvent.objects.count(), 4, "Additional runs do not recreate the same beam; 4 total")
        d_event = SponsorEvent.objects.get(external_id__contains='UCd')
        c_event = SponsorEvent.objects.get(external_id__contains='UCc')
        self.assertTrue(c_event.id < d_event.id, 'Insert new beam in reverse order')
