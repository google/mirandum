...
from patreon.models import SponsorUpdate, SponsorEvent
from googaccount.models import AppCreds
from django.contrib.auth.models import User

from patreon.support import run_sponsors
...
class InsertPatreon(TestCase):
    ...
    def testInsertion(self):
        ...
        def sponsor_data(*args):
            return create_patreon(['a', 'b'])
        run_patreon(self.updater, sponsor_data)
        
        def sponsor_data(*args):
            return create_patreon(['d', 'c', 'a', 'b'])

        run_patreon(self.updater, sponsor_data)
        self.assertEqual(SponsorEvent.objects.count(), 4, "Additional runs do not recreate the same patreon; 4 total")
        d_event = SponsorEvent.objects.get(external_id__contains='UCd')
        c_event = SponsorEvent.objects.get(external_id__contains='UCc')
        self.assertTrue(c_event.id < d_event.id, 'Insert new patreon in reverse order')
