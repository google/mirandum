...
from twitchfollows.models import TwitchFollowEvent, TwitchFollowAlertConfig
import json
import random
...
@receiver(post_save, sender=TwitchFollowEvent)
def event(instance, **kwargs):
    user = instance.updater.credentials.user
    alerts = TwitchFollowAlertConfig.objects.filter(user=user)
